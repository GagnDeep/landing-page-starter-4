...
export function LogoMarquee({ className, logos, speed = 40 }: LogoMarqueeProps) {
  ...
  if (shouldReduceMotion) {
    return (
      <div className={cn("flex flex-wrap justify-center gap-8 py-8 opacity-50 grayscale", className)}>
        {logos.map((logo, idx) => (
          /* eslint-disable-next-line @next/next/no-img-element */
          <img key={`static-${idx}`} src={logo.src} alt={logo.alt} className="h-12 w-auto object-contain" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div
      className={cn(
        "relative flex w-full overflow-hidden py-12 mask-image-fade-horizontal",
        className
      )}
      style={{
    ...
    >
      <motion.div
        className="flex w-max flex-nowrap gap-16 pr-16 opacity-60 grayscale hover:grayscale-0 transition-all duration-500"
        animate={{
      ...
      >
        {scrollContent.map((logo, idx) => (
          <div key={`marquee-${idx}`} className="flex items-center justify-center relative h-10 w-32">
            {/* eslint-disable-next-line @next/next/no-img-element */}
