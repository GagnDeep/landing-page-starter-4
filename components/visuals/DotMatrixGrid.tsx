...
export function DotMatrixGrid({
  ...
  const getMaskStyle = () => {
    switch (maskDirection) {
      ...
      case "center":
        return { maskImage: "radial-gradient(circle at center, black, transparent 80%)" };
      case "none":
      default:
        return {};
    }
  ...
  return (
    <div
      className={cn("pointer-events-none absolute inset-0 z-[-1] opacity-30", className)}
      ...
      >
        <defs>
          <pattern id={patternId} width={size} height={size} patternUnits="userSpaceOnUse">
            <circle cx={dotSize / 2} cy={dotSize / 2} r={dotSize / 2} fill={dotColor} />
          </pattern>
        </defs>
        {/* We make the rect slightly larger than 100% to ensure smooth infinite scrolling without cutting off */}
        <rect x="0" y="0" width="100%" height="200%" fill={`url(#${patternId})`} />
      </motion.svg>
    </div>
  );
}
