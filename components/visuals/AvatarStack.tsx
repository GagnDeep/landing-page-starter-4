    ...
    >
      {displayAvatars.map((avatar, idx) => (
        <motion.div
          ...
          className={cn(
            "relative overflow-hidden rounded-full border-background shadow-sm",
            sizeClasses[size],
      ...
      {excessCount > 0 && (
        <motion.div
          variants={shouldReduceMotion ? undefined : itemVariants}
          className={cn(
            "relative z-0 -ml-3 flex items-center justify-center rounded-full border-background bg-muted font-medium text-muted-foreground shadow-sm",
            sizeClasses[size]
          )}
        >
          +{excessCount}
