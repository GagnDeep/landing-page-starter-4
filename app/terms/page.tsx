import { termsContent } from "@/content/terms";
import { PageHero } from "@/components/ui/page-hero";
import { Prose } from "@/components/ui/prose";

export const metadata = {
  title: termsContent.seo.title,
  description: termsContent.seo.description,
};

// Fix parser to ensure valid HTML structure without nesting block elements in <p>
function parseMarkdown(text: string) {
  let html = text.replace(/### (.*)/g, "<h3>$1</h3>");
  html = html.replace(/\n\n/g, "<br/><br/>");
  return `<div>${html}</div>`;
}

export default function TermsPage() {
  const content = termsContent.sections.content;

  return (
    <>
      <PageHero
        headline={termsContent.sections.hero.headline || "Terms of Service"}
        subheading={termsContent.sections.hero.subheading}
      />
      <Prose dangerouslySetInnerHTML={{ __html: parseMarkdown(content.body || "") }} />
    </>
  );
}
